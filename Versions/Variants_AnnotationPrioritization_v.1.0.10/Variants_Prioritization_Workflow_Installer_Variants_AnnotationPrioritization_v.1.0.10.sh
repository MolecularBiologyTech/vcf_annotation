...
# VEP Docker paths
DOCKER_DATA="$TOOLS_DIR/docker-data"
VEP_DATA="$TOOLS_DIR/vep_data"
PLUGIN_DATA="$TOOLS_DIR/plugin_data"
...
GNOMAD_COVERAGE_DIR="$PLUGIN_DATA/gnomad/coverage"
GNOMAD_CONSTRAINT_DIR="$PLUGIN_DATA/gnomad/constraint"
...
# BaseSpace CLI variables
BASESPACE_DIR="$TOOLS_DIR/BaseSpaceCLI"
...
###############################################
# AUTHENTICATION PHASE - All password/authentication upfront
###############################################
echo "=========================================="
echo "PHASE 1: SYSTEM DEPENDENCIES (sudo required)"
echo "=========================================="
...
echo "=========================================="
echo "PHASE 2: DOCKER INSTALLATION + CONFIGURATION (sudo required)"
echo "=========================================="
...
###############################################
# Configure Docker data-root and permissions
###############################################
mkdir -p "$DOCKER_DATA"
sudo chown -R "$USER":"$USER" "$DOCKER_DATA"

echo "Configuring Docker data-root to: $DOCKER_DATA"
...
echo "=========================================="
echo "PHASE 3: DIRECTORY PERMISSIONS (sudo required)"
echo "=========================================="

###############################################
# Setup directories and permissions
###############################################
mkdir -p "$VEP_DATA" "$PLUGIN_DATA"
mkdir -p "$PLUGIN_DATA"/{dbnsfp,spliceai,loftee,gnomad,curated_lof}
mkdir -p "$GNOMAD_COVERAGE_DIR" "$GNOMAD_CONSTRAINT_DIR"
...
echo "=========================================="
echo "PHASE 4: IGV DEPENDENCIES (sudo required)"
echo "=========================================="

###############################################
# IGV dependencies (moved here for early authentication)
###############################################
sudo apt install -y python3 python3-pip wget unzip xvfb x11-utils default-jre openjdk-8-jre

echo "=========================================="
echo "PHASE 5: BASESPACE AUTHENTICATION (interactive)"
echo "=========================================="

###############################################
# BaseSpace CLI + Authentication (moved early)
###############################################

mkdir -p "$BASESPACE_DIR"
...
if [ ! -f "$HOME/.basespace/default.cfg" ]; then
    echo "Please authenticate with your BaseSpace account..."
...
echo "=========================================="
echo "PHASE 6: DOCKER PULL"
echo "=========================================="

###############################################
# Docker pull (moved early for authentication)
###############################################
...
echo "Remaining installation will run unattended."
echo "=========================================="

###############################################
# NON-AUTHENTICATION PHASE - Unattended installation
###############################################
...
###############################################
# 2.5. VEP 115 Setup (Docker-based)
###############################################

echo "=========================================="
echo "Setting up VEP 115 + Plugins"
echo "=========================================="

###############################################
# Docker configuration and directories already done in auth phase
###############################################

###############################################
# SpliceAI Download (authentication already done)
###############################################

echo "=========================================="
echo "Downloading SpliceAI score files via BaseSpace"
echo "=========================================="

for ID in "${FILES[@]}"; do
    echo "Downloading SpliceAI file ID: $ID"
    "$BASESPACE_BIN" download file --id "$ID" --output "$OUTDIR"
done

echo "SpliceAI download complete."
echo "SpliceAI files installed at: $OUTDIR"
...
echo "=========================================="
echo "IGV installation complete"
echo "=========================================="

###############################################
# VEP 115 INSTALL (CACHE INTO VEP_DATA)
###############################################
# Docker pull already done in auth phase
...
echo "VEP cache installation complete."
...
wget -q -O LOEUF.pm https://raw.githubusercontent.com/Ensembl/VEP_plugins/release/115/LOEUF.pm

chmod a+r LoFtool.pm LoFtool_scores.txt SpliceAI.pm dbNSFP.pm dbNSFP_replacement_logic gnomADc.pm LOEUF.pm
...
###############################################
# REMOVED: Joint Frequency (redundant with gnomADc VCF)
###############################################
echo "Joint Frequency data skipped - redundant with gnomADc VCF plugin"

###############################################
# gnomAD v4.1.1 CONSTRAINT
###############################################

echo "=========================================="
echo "Downloading gnomAD v4.1.1 Constraint"
echo "=========================================="

mkdir -p "$GNOMAD_CONSTRAINT_DIR"
cd "$GNOMAD_CONSTRAINT_DIR"

wget -c "https://storage.googleapis.com/gcp-public-data--gnomad/release/${GNOMAD_VERSION}/constraint/gnomad.v${GNOMAD_VERSION}.constraint.tsv.gz"

echo "gnomAD Constraint download complete."

###############################################
# gnomAD v4.1.1 LOEUF (Pre-processed for VEP)
###############################################

echo "=========================================="
echo "Downloading pre-processed LOEUF dataset for VEP"
echo "=========================================="

cd "$GNOMAD_CONSTRAINT_DIR"

wget -c "https://ftp.ensembl.org/pub/current/variation/gnomADc/loeuf_dataset_grch38.tsv.gz"

echo "Indexing LOEUF dataset..."
tabix -s 1 -b 2 -e 2 loeuf_dataset_grch38.tsv.gz

echo "LOEUF dataset download and indexing complete."

###############################################
# gnomAD v4.1.1 COVERAGE
###############################################

echo "=========================================="
echo "Downloading gnomAD v4.1.1 Coverage"
echo "=========================================="

mkdir -p "$GNOMAD_COVERAGE_DIR"
cd "$GNOMAD_COVERAGE_DIR"

wget -c "https://storage.googleapis.com/gcp-public-data--gnomad/release/${GNOMAD_VERSION}/coverage/genomes/gnomad.genomes.v${GNOMAD_VERSION}.coverage.summary.tsv.bgz"
wget -c "https://storage.googleapis.com/gcp-public-data--gnomad/release/${GNOMAD_VERSION}/coverage/genomes/gnomad.genomes.v${GNOMAD_VERSION}.coverage.summary.tsv.bgz.tbi"

echo "Processing coverage file for VEP compatibility..."
gunzip -c gnomad.genomes.v${GNOMAD_VERSION}.coverage.summary.tsv.bgz | sed '1s/.*/#&/' > gnomad.genomes.tabbed.tsv
sed "1s/locus/chr\tpos/; s/:/\t/g" gnomad.genomes.tabbed.tsv > gnomad.ch.genomesv4.tabbed.tsv
bgzip -f gnomad.ch.genomesv4.tabbed.tsv
tabix -f -s 1 -b 2 -e 2 gnomad.ch.genomesv4.tabbed.tsv.gz
rm gnomad.genomes.tabbed.tsv gnomad.ch.genomesv4.tabbed.tsv

echo "gnomAD Coverage download and processing complete."
...
echo "gnomAD v4.1.1 genomes installed at: $GNOMAD_DIR"
echo "gnomAD v4.1.1 coverage installed at: $GNOMAD_COVERAGE_DIR"
echo "gnomAD v4.1.1 constraint + LOEUF installed at: $GNOMAD_CONSTRAINT_DIR"
echo "gnomAD curated LoF installed at: $CURATED_DIR"
...
echo "VEP plugins: LoF, LoFtool, SpliceAI, dbNSFP, gnomADc, LOEUF"
...
if [ -z "$BASE" ]; then
    echo "ERROR: BASE variable not set in 1_Define_data_specs.txt"
    echo "Please ensure the installation was completed correctly"
    exit 1
fi

TOOLS_DIR="$BASE/tools"
echo "Installation directory (BASE): $BASE"
...
indel_spliceai_masked=/plugins/spliceai/spliceai_scores.masked.indel.hg38.vcf.gz \
      ...
      \
      --plugin gnomADc,/plugins/gnomad/coverage/gnomad.ch.genomesv4.tabbed.tsv.gz \
      \
      --plugin LOEUF,file=/plugins/gnomad/constraint/loeuf_dataset_grch38.tsv.gz,match_by=gene \
      \
...
# Copy README to BASE directory
cp "$(dirname "$0")/README.md" "$BASE/README.md"
...
echo "=============================================="
echo "Next steps:"
echo "  1. Review $BASE/README.md for installation and usage details"
