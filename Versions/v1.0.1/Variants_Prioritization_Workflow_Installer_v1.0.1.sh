...
# Test change - adding comment for version testing
###############################################
...
###############################################
# 9. Install Docker + VEP (Docker-based)
###############################################
...
echo "=========================================="
echo "Installing Docker + Preparing VEP + Plugins"
echo "=========================================="
...
###############################################
# Create docker-data with correct permissions
###############################################
mkdir -p "$DOCKER_DATA"
sudo chown -R $USER:$USER "$DOCKER_DATA"
chmod -R 775 "$DOCKER_DATA"

###############################################
# Move Docker storage to BASE
###############################################
...
###############################################
# Prepare VEP data directory
###############################################
mkdir -p "$VEP_DATA"
sudo chown -R $USER:$USER "$VEP_DATA"
chmod -R 775 "$VEP_DATA"

###############################################
# Download VEP cache + FASTA (official)
###############################################
echo "=========================================="
echo "Downloading VEP cache + FASTA (GRCh38)..."
echo "=========================================="
...
###############################################
mkdir -p "$PLUGIN_DATA"/{cadd,dbnsfp,spliceai,revel,alphamissense,loftee,opentargets,gnomad_v4}
...
wget ftp://dbnsfp:dbnsfp@dbnsfp.softgenetics.com/dbNSFPv5.3.1.zip
unzip dbNSFPv5.3.1.zip

###############################################
# SpliceAI
###############################################
echo "Downloading SpliceAI..."
cd $PLUGIN_DATA/spliceai
wget https://storage.googleapis.com/broad-ml4cv-public/SpliceAI/GRCh38/spliceai_scores.raw.snv.hg38.vcf.gz
wget https://storage.googleapis.com/broad-ml4cv-public/SpliceAI/GRCh38/spliceai_scores.raw.indel.hg38.vcf.gz

###############################################
# REVEL
###############################################
echo "Downloading REVEL..."
cd $PLUGIN_DATA/revel
wget https://rothsj06.u.hpc.mssm.edu/revel/revel_all_chromosomes.csv.gz

###############################################
# AlphaMissense
###############################################
echo "Downloading AlphaMissense..."
cd $PLUGIN_DATA/alphamissense
wget https://storage.googleapis.com/alphamissense/AlphaMissense_hg38.tsv.gz

###############################################
# LOFTEE
###############################################
echo "Downloading LOFTEE..."
cd $PLUGIN_DATA/loftee
wget https://raw.githubusercontent.com/konradjk/loftee/master/human_ancestor.fa.gz
wget https://raw.githubusercontent.com/konradjk/loftee/master/conservation.bw

###############################################
# OpenTargets
###############################################
echo "Downloading OpenTargets..."
cd $PLUGIN_DATA/opentargets
wget https://storage.googleapis.com/open-targets-data-releases/vep_plugin/opentargets_data.json.gz

###############################################
# gnomAD v4
###############################################
echo "Downloading gnomAD v4..."
cd $PLUGIN_DATA/gnomad_v4
wget https://gnomad.broadinstitute.org/downloads/gnomad.genomes.v4.0.sites.vcf.bgz
wget https://gnomad.broadinstitute.org/downloads/gnomad.genomes.v4.0.sites.vcf.bgz.tbi

echo "=========================================="
echo "INSTALLATION COMPLETE"
echo "=========================================="
echo "Docker storage: $DOCKER_DATA"
echo "VEP cache + FASTA: $VEP_DATA"
echo "Plugin datasets: $PLUGIN_DATA"
...
# Generate Workflow_Explained.md
cat > "$BASE/Workflow_Explained.md" << 'ENDOFWORKFLOW'
...
The installation script (Variants_Prioritization_Workflow_Installer_10.06.2026.sh) automatically:
...
6. **Creates documentation** (Workflow_Explained.md)
...
echo "Created: $BASE/Workflow_Explained.md"
...
echo "=============================================="
echo "Next steps:"
echo "  1. Review $BASE/0_README_Trio_Analysis_Workflow.md for complete workflow documentation"
echo "  2. Edit $BASE/1_Define_data_specs.txt with your configuration"
echo "  3. Run: cd $BASE && ./2_Run_analysis.sh"
echo "=============================================="