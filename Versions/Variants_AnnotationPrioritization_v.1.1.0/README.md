...
3. Run with sudo:
```bash
sudo bash Versions/v.1.0.9/Variants_Prioritization_Workflow_Installer_v.1.0.9.sh
```
...
The installation script (Variants_Prioritization_Workflow_Installer_v.1.0.9.sh) automatically:
...
| **gnomAD v4.1.1** | Latest population frequency data from large-scale sequencing projects |
| **gnomAD v4.1.1 Coverage** | Position-level depth information for quality assessment |
| **gnomAD v4.1.1 Constraint (LOEUF)** | Gene-level constraint scores (pLI, LOEUF) for loss-of-function intolerance |
...
| **gnomAD v4.1.1 AF** | Latest population frequencies (v4.1.1) | Updated rarity filtering |
| **gnomAD Coverage** | Position-level depth information | Quality assessment of frequency data |
| **LOEUF** | Gene-level constraint scores (pLI, LOEUF) | Gene intolerance assessment for LoF variants |
